  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <main className="flex-1 max-w-md mx-auto w-full p-4 space-y-4">
        {isLoading ? (
          <p className="text-neutral-400 text-center">Loading your workout history...</p>
        ) : workouts && workouts.length > 0 ? (
          workouts.map((workout) => (
            <Card 
              key={workout.id} 
              className="rounded-xl border border-neutral-800 bg-background"
            >
              ...
              <CardContent>
                <ul className="space-y-2">
                  {workout.workout_exercises?.map((workoutExercise, index) => (
                    <li 
                      key={index}
                      className="flex justify-between items-center text-sm py-2 border-b border-neutral-700 last:border-0"
                    >
          ...
          <div className="text-center py-10">
            <p className="text-neutral-400">You haven't recorded any workouts yet</p>
            <Link 
              to="/record-workout" 
